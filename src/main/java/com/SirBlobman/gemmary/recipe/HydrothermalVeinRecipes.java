...
public class HydrothermalVeinRecipes {
    private static HydrothermalVeinRecipes INSTANCE;
    private final Map<ItemStack, ItemStack> recipeMap = new HashMap<>();
    private HydrothermalVeinRecipes() {
        ItemStack berylliumItem = new ItemStack(GemmaryItems.BERYLLIUM, 1);
        ItemStack emeraldPartItem = new ItemStack(GemmaryItems.EMERALD_PART, 1);
        addRecipe(berylliumItem, emeraldPartItem);
    }
    ...
    public static HydrothermalVeinRecipes getInstance() {
        if(INSTANCE == null) INSTANCE = new HydrothermalVeinRecipes();
    ...
    public void addRecipe(ItemStack input, ItemStack output) {
        ...
        if(resultItem != ItemStack.EMPTY) {
            String errorMessage = String.format("Ignored hydrothermal vein recipe with conflicting input %s = %s", input, output);
