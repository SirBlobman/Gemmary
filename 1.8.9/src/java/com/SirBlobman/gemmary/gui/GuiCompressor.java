...
//GuiCompressor takes recipes from the class and turns them into diamonds.
@SideOnly(Side.CLIENT)
public class GuiCompressor extends GuiContainer {

	// This is the resource location for the background image
	private static final ResourceLocation texture = new ResourceLocation("textures/gui/compressor2.png");
	private CCTE tileEntity;

	public GuiCompressor(InventoryPlayer invPlayer, CCTE tileCompressor) {
		super(new ContainerCompressor(invPlayer, tileCompressor));
	...
	protected void drawGuiContainerBackgroundLayer(float partialTicks, int x, int y) {
		...
		for (int i = 0; i < CCTE.FUEL_SLOTS_COUNT; ++i) 
        {
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		...
		if (isInRect(guiLeft + COOK_BAR_XPOS, guiTop + COOK_BAR_YPOS, COOK_BAR_WIDTH, COOK_BAR_HEIGHT, mouseX, mouseY)){
			hoveringText.add(I18n.format("gui.cc.cooktime", new Object[0]));
		...
		for (int i = 0; i < CCTE.FUEL_SLOTS_COUNT; ++i) {
			if (isInRect(guiLeft + FLAME_XPOS + FLAME_X_SPACING * i, guiTop + FLAME_YPOS, FLAME_WIDTH, FLAME_HEIGHT, mouseX, mouseY)) {
				hoveringText.add(I18n.format("gui.cc.fuel", new Object[0]));
