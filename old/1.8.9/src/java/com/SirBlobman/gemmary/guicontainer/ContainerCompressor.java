...
public class ContainerCompressor extends Container {
	...
	// slot index is the unique index for all slots in this container i.e. 0 - 35 for invPlayer then 36 - 49 for tileCompressor
	...
	// slot number is the slot number within each component; i.e. invPlayer slots 0 - 35, and tileCompressor slots 0 - 14
	...
	public ContainerCompressor(InventoryPlayer invPlayer, CCTE tileCompressor) 
	{
		...
		for (int x = 0; x < FUEL_SLOTS_COUNT; x++) {
			...
			addSlotToContainer(new SlotFuel(tileCompressor, slotNumber, FUEL_SLOTS_XPOS + SLOT_X_SPACING * x, FUEL_SLOTS_YPOS));
		}
		...
		for (int y = 0; y < INPUT_SLOTS_COUNT; y++) {
			...
			addSlotToContainer(new SlotSmeltableInput(tileCompressor, slotNumber, INPUT_SLOTS_XPOS, INPUT_SLOTS_YPOS+ SLOT_Y_SPACING * y));
		}
		...
		for (int y = 0; y < OUTPUT_SLOTS_COUNT; y++) {
			...
			addSlotToContainer(new SlotOutput(tileCompressor, slotNumber, OUTPUT_SLOTS_XPOS, OUTPUT_SLOTS_YPOS + SLOT_Y_SPACING * y));
		}
	}
	...
	{
		return tileCompressor.isUseableByPlayer(player);
	}
	...
	public void detectAndSendChanges() {
		...
		boolean fieldHasChanged [] = new boolean[tileCompressor.getFieldCount()];
		if (cachedFields == null) {
			cachedFields = new int[tileCompressor.getFieldCount()];
			allFieldsHaveChanged = true;
		}
		for (int i = 0; i < cachedFields.length; ++i) {
			if (allFieldsHaveChanged || cachedFields[i] != tileCompressor.getField(i)) {
		...
		for (int i = 0; i < this.crafters.size(); ++i) {
			...
			for (int fieldID = 0; fieldID < tileCompressor.getFieldCount(); ++fieldID) {
	...
	public void updateProgressBar(int id, int data) {
		tileCompressor.setField(id, data);
	}
	...
	public class SlotFuel extends Slot {
		...
		public boolean isItemValid(ItemStack stack) {
			return CCTE.isItemValidForFuelSlot(stack);
		}
	}

	// SlotSmeltableInput is a slot for input items
	public class SlotSmeltableInput extends Slot {
		public SlotSmeltableInput(CCTE tileCompressor, int index, int xPosition, int yPosition) {
			super(tileCompressor, index, xPosition, yPosition);
		}

		// if this function returns false, the player won't be able to insert the given item into this slot
		@Override
		public boolean isItemValid(ItemStack stack) {
			return CCTE.isItemValidForInputSlot(stack);
		}
	}
	...
	public class SlotOutput extends Slot {
		...
		@Override
		public boolean isItemValid(ItemStack stack) {
			return CCTE.isItemValidForOutputSlot(stack);
		}
	}
}